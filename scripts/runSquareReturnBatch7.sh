#!/usr/bin/env bash
# Batch R7 — 18 multi-quantity / unit-level refund TCs (36 interactions).
#
# Usage:
#   ./scripts/runSquareReturnBatch7.sh
#   TAG=batchr7 RESUME='BatchR7MultiQty3OneUnitRefund:5' ./scripts/runSquareReturnBatch7.sh
set -euo pipefail
cd "$(dirname "$0")/.."

export SUITE="Square_Suite/Return_Import"
export TAG="${TAG:-batchr7}"
...
LOG="/tmp/square_return_batch7.log"

echo "=============================================="
echo " Square Return/Refund — Batch R7 (18 TCs)"
...
node scripts/generateSquareReturnBatch7.js
